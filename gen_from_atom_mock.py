...
def _build_split_plan(num_items: int, ratios: tuple[int, int, int]) -> list[str]:
    """num_items 개의 scene을 주어진 비율로 train/val/test에 배분하는 계획을 생성합니다."""
    total = sum(ratios)
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Split multi-page TIF (8ms, 10ms repeating) into "
    ...
    parser.add_argument(
        ...
        help="Shuffle scene order (pairs) before splitting."
    )
    parser.add_argument(
    ...
    with tifffile.TiffFile(args.tif_path) as tif:
        ...
        # 8ms, 10ms 2개 단위 반복 가정
        group_size = 2
        if num_pages % group_size != 0:
            raise ValueError(
                f"Input TIF has {num_pages} pages which is not divisible by 2.\n"
                "This script assumes frames come in 8ms, 10ms repeating pairs."
            )

        num_scenes = num_pages // group_size

        # 각 scene은 (8ms_idx, 10ms_idx) 인덱스 튜플
        scene_pairs = [
            (2 * i, 2 * i + 1) for i in range(num_scenes)
        ]
        ...
        for offset, pair in enumerate(scene_pairs):
